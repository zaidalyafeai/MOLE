...
files += glob("/ibex/ai/home/alyafez/sintetis/.cache/sint-output/**/**.jsonl")
output_path = "static/synth_slot_reversed_papersv2"
os.makedirs(output_path, exist_ok=True)
total_items_created = 0
for file in tqdm(files):
    data = open(file, "r").readlines()
    if not any([pattern in file for pattern in ["0043f4d24a"]]):
        continue
    print(len(data))
    for line in tqdm(data):
        ...
        if "8a66e47c5b" in file:
            slot_type = "slot"
        elif "0043f4d24a" in file:
            slot_type = "slot-reversedv3"
        
        # if "3f35b74824" in file:
        #     link = f"papers_{data['model']}_{slot_type}_{data['id']}"
        if "ec64585fcf" in file:
            link = f"docling_{data['model']}_{slot_type}_{data['id']}"
        elif "e3439af9f0" in file or "0043f4d24a" in file:
            link = f"fineweb_{data['model']}_{slot_type}_{data['id']}"
        elif "8a66e47c5b":
            link = f"dummy_{data['model']}_{slot_type}_{data['id']}"
        else:
            continue
        ...
        if "docling" in link:
            # print(folder_name)
            with open(f"static/papers/{folder_name}/paper_docling.txt", "w") as f:
                f.write(text)
        else:
