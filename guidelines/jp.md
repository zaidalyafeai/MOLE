...
6. **Language**: the language of the dataset.
...
11. **Volume**: The volume is the total number of samples in the dataset.
...
18. **Script**: The script of the dataset.
19. **Tokenized**: Is the dataset lemmatized or stemmed?
20. **Host**: you can use the Link attribute to write the repository where the dataset is hosted. Use other if the host is not from the given options. 
21. **Access**: Free if the dataset is free. Upon Request, if the dataset is behind a form. With-Fee if the dataset is paid. 
22. **Cost**: the cost of the dataset if paid. 
23. **Test_Split**: is true only if the dataset has (training and test splits). If the dataset has only one split, even if it is for testing, we set that to false
24. **Tasks**: the list of tasks that this dataset is intended for. Use other if the task doesn’t exist in the options.
25. **Venue_Title**: full name of the venue, for arXiv we don't use the full name.
26. **Venue_Type**: type, either preprint, workshop, conference, or journal.
27. **Venue_Name**: if the dataset is from arXiv and there is no mention of the conference workshop, etc. in the paper. If the paper has something like (paper published in ACL), then we put Venue Title = ACL, Venue Type = conference, and Venue Name = Association of Computational Linguistics.
28. **Authors**: the authors of the paper in an ordered fashion.
29. **Affiliations**: list only the affiliations without repetition
30. **Abstract**: the full abstract of the paper. 